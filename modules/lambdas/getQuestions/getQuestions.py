...
def lambda_handler(event, context):
    try:
        ...
        logger.info(f"Consultando preguntas con PK={pk}")

        # Query usando KeyConditionExpression para PK y filtro begins_with en SK
        response = table.query(
            KeyConditionExpression=boto3.dynamodb.conditions.Key('PK').eq(pk) & 
                                   boto3.dynamodb.conditions.Key('SK').begins_with('QUESTION#')
        )

        items = response.get('Items', [])

        logger.info(f"Encontradas {len(items)} preguntas")
        ...
        logger.error(f"Error ClientError consultando preguntas: {str(e)}")
        return {
            'statusCode': 500,
            'body': json.dumps({'error': str(e)})
        }
    except Exception as e:
        logger.error(f"Error inesperado consultando preguntas: {str(e)}")
        return {
            'statusCode': 500,
            'body': json.dumps({'error': str(e)})
        }
